...
{
    public class OrdersCreatedSpecificationTest
    {
        [Fact]
        public void MatchesExpectedNumberOfOrders()
        {
            var spec = new OrdersCreatedSpecification();
        ...
        public static List<Order> GetTestOrdersCollection()
        {
            ...
            orders[1].SetCleanerId("1"); // InProgress

            orders[2].SetCleanerId("1");
            orders[2].Cancel(); // Cancelled

            orders[3].SetCleanerId("1");
            orders[3].Close(); // Closed
